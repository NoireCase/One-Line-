...
常用可复制任务模板已单独整理到 [`docs/codex-task-templates.md`](./codex-task-templates.md)。后续新增模板优先写入该文件，本文只保留流程原则与任务分级。
...
以下规则来自 2026-07-01 至 2026-07-08 的 Codex 协作复盘，作为后续默认执行原则：

- 高风险任务先只读审查，再确认后修改。高风险包括玩法、solver、validator、关卡数据、存档、评分、解锁和 release。
- UI 小改必须显式禁止修改玩法规则、solver、validator、关卡数据、存档结构和评分逻辑。
- Git / Release 必须单独成任务，不能和开发、文档、测试修改混在同一轮。
- Validator 应尽量覆盖 runtime 的关键规则，不能只检查数据形状。
- 完成任务时必须报告修改文件、是否影响玩法 / 存档 / 关卡、验证命令结果和 git status。
