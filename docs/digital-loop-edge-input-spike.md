...
P4B 验收表（**已回填：P4B 桌面最终人工验收与聚焦测试于 2026-08-01 全部通过，见 `src/prototypes/digitalLoop/PROTOTYPE.md`「人工验收归档」**）：

| # | 验收项 | 判定方式 | 当前状态 |
| --- | --- | --- | --- |
| 1 | pointermove 高频输入下无明显掉帧 | 拖动期间人工观察 + 可选 rAF/帧率采样 | **通过**（人工验收 + 压力场景聚焦测试，未见明显掉帧） |
| 2 | 一次手势只产生一次 undo | 诊断场景断言 | 合同已冻结 ✅ |
| 3 | 不发生相邻 edge 批量误选 | 诊断场景命中断言 + 人工试玩 | **通过**（cell / clue / 平行边中间安全区断言 + 人工验收） |
| 4 | 拖动结果可预测 | 人工试玩（鼠标/触摸板）；起点决定模式行为恒定 | **通过**（人工验收；45° / 多转角 / 高速过顶点补充证据） |
| 5 | pointercancel 可恢复 | 模拟 pointercancel 后界面与撤销栈一致 | 合同已冻结 ✅（含 lostpointercapture 证据） |
| 6 | 切换视图后无残留 gesture | 卸载原型页面后无事件监听器泄漏、无未提交批 | 合同已冻结 ✅（P2-3 生命周期） |
| 7 | 10×10、11×11 可正常操作 | 压力棋盘人工操作 + 结构判定正确 | **通过**（压力场景 + 人工验收） |
| 8 | 移动端不触发页面滚动或浏览器手势冲突 | **移出当前阶段**（移动端按平台政策整体暂缓；`touch-action: none` 保持仅作用原型棋盘容器） | 已移出 |
| 9 | `line` 手势不误写 `excluded`，`excluded` 输入不误写 `line` | 三态诊断场景断言 + 人工试玩 | **通过**（Line 优先于 X 已冻结；三态诊断 + 人工验收） |
| 10 | 数字线索计数与冲突提示正确 | 最小 Loopy 场景断言（场景 6–11） | 合同已冻结 ✅ |
| 11 | 完成判定 = 单环 ∧ 全部数字满足 | 场景 10、11 断言未完成；单环全满足场景断言完成 | 合同已冻结 ✅（场景 14 完成正例） |
| 12 | 不出现正式存档和进度写入 | 断言无任何 `cg_*` 正式 key 写入（原型 key 除外） | 合同已冻结 ✅ |
| 13 | 删除原型后正式 build 仍成立 | 删除 `src/prototypes/` 后 `npm run build` 通过 | 合同已冻结 ✅（原型为可整体删除叶子依赖） |
