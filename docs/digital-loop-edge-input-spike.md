...
- One-Line **不直接复制其桌面控制方式**（右键排除等桌面约定在移动端不成立，输入方式必须重新设计并实测）。
...
**P4B 至少比较以下输入方案（P4A 不提前宣告胜出）：**

| 方案 | 描述 | 适用性 |
| --- | --- | --- |
| A. 桌面参考方案 | 左键拖动 = `line`；右键 = `excluded`（类 Loopy 桌面约定） | 桌面鼠标可行；触摸板/移动端需验证右键等价操作；移动端不成立 |
| B. 统一工具方案 | 当前工具为 `line` / `excluded` / `erase`；点击和拖动按当前工具执行 | 全平台一致；多一步工具切换；撤销语义需记录工具状态 |
| C. 移动端手势方案 | 点击或拖动 = `line`；长按、双击或工具按钮 = `excluded` | 移动端与触摸板候选；长按/双击的误触率与可发现性待测 |

P4B 必须基于鼠标、Mac 触摸板、移动触摸、390×844 视口、10×10 与 11×11 棋盘做实际选择；最终输入方案 = 实测结果 + 人工试玩记录，不在此处预判。
...
- 手势期间**不因经过其他状态自动切换模式**：添加模式经过已存在的 `line` 或 `excluded` 边时保持该边不变；擦除模式经过 `undecided` 或 `excluded` 边时不新增线也不修改它们。
- 每个 edge 同一手势只处理一次（第 7 节核心合同 2）。
- 起始点在死区（顶点附近）或未命中任何边时，手势不启动，不产生任何变更。
- **`excluded` 的标记与取消不通过拖动自动产生**：它属于独立输入通道（方案 A 的右键、方案 B 的工具、方案 C 的长按/双击/按钮之一），由 P4B 实测选择；`excluded` 边作为手势起点的行为（进入/擦除该边）属于 P4B 待测，不在此冻结。
...
| 结论 | 含义 |
| --- | --- |
| **GO** | 输入可靠、性能可接受、桌面和移动端均可操作、两层判定正确、原型隔离成立；允许在后续工程包开发完整数字环线原型 |
...
- 三态 edge input 可用。
- `excluded` 状态在桌面和移动端都有**可理解**的输入方案。
- 单环结构判定正确（第一层）。
- 最小数字线索判定正确（第二层）。
- 单环与数字线索**联合决定完成**。
- 通用输入层未硬编码 Loopy 规则。
...
| 10 | 移动端 excluded 无可用输入 | 排除标记在触摸设备上不可操作 | 方案 A/B/C 实测比较（第 7 节） | 无可用方案 → GO WITH CHANGES 或 NO-GO |
| 11 | DOM 命中与数据坐标漂移 | 渲染缩放后命不中 | 多视口命中断言 | 漂移 → GO WITH CHANGES（坐标换算） |
| 12 | 视口缩放导致 hit corridor 失真 | 不同视口手感不一致 | 390×844 / 1440×900 对比测试 | 失真 → GO WITH CHANGES |
