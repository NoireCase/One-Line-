// Production Edge Puzzle Foundation · Pointer 连续笔划控制器（纯逻辑，无 DOM / 无 React）
// 桌面最终输入映射（Line 优先于 X）：
...
//
// P2-1 按钮 Guard：正式入口只接受 button 0（primary）与 button 2（secondary）；
//   button 1（中键）、button 3 / 4（侧键）立即忽略：不启动手势、不修改 Edge、
//   不创建 transaction、不进入 Undo、不产生 preview。
//
...
//
// Shift+左键连续笔划模型：
//   pointerdown 确定通道与潜在操作 → 移动超过阈值（屏幕 CSS 像素）进入拖动 →
//   记录连续 Pointer 轨迹 → 按顺序对 A→B 线段采样 → 采样点统一 hit-testing →
//   P2-2 严格相邻：只接受与上一条已接受 Edge 共享顶点的 Edge；无相邻候选 reject（不回退全候选池）→
//   顶点按连续性 + 移动方向裁决 → 整个笔划一次提交。
//
// 核心合同：
// 1. 一次 pointerdown → pointerup = 一个 undo step（右键单击同样只产生一步）。
// 2. 同一 Edge 同一手势只修改一次（visited 去重）。
// 3. pointercancel / blur / Esc 整体回滚或清理，不入栈。
...
// 8. 本控制器不包含 React state、诊断场景、clue、completion、debug UI 或 Undo stack。
...
// P2-1：正式入口接受的按钮（primary / secondary）；其余按钮一律忽略
const ACCEPTED_BUTTONS = new Set([0, 2]);
...
// 两条 Edge 是否相同或共享顶点（P2-2 严格相邻的唯一依据）
 ...
 * 创建手势控制器。
 * @param {object} opts
 * @param {object} opts.layout computeBoardLayout 输出（含 n、cellSize、corridorHalfWidth、tieEpsilon）
 ...
 * @param {(event: object) => void} opts.onStrokeDebug 可选：事件流通知（不含任何渲染）
...
export function createGestureController({
  ...
  const handlePointerDown = ({ local, screen, pointerId, button, shiftKey }) => {
    ...
    // P2-1 按钮 Guard：仅 primary（0）与 secondary（2）；中键/侧键等一律忽略
    if (!ACCEPTED_BUTTONS.has(button)) return;
  ...
  // P2-2 严格相邻：只在与上一条已接受 Edge 共享顶点的候选中裁决；无相邻候选返回 null（reject）。
  const pickCandidate = (detail, moveDir, prevKey) => {
    ...
    // 严格相邻：候选池只保留与上一条 Edge 相同或共享顶点的边
    const connected = candidates.filter((c) => edgesConnected(prevKey, c.key));
    if (connected.length === 0) return null; // 无相邻候选：reject，不回退全候选池

    if (connected.length === 1) return connected[0].key;

    // 多个相邻候选：与移动方向更一致者优先（边方向与移动方向点积）
  ...
  /**
   * Esc / 外部取消（含 P2-3：lostpointercapture、controller 销毁、Reset、场景切换）：
   * 等价 pointercancel（不要求 pointerId 匹配）。
   * 回滚全部未提交变更、清理右键 pending。
   * @returns {boolean} 是否存在被取消的活动状态
   */
  const cancelActive = () => {
    if (pending.active) {
      resetPendingRightClick();
