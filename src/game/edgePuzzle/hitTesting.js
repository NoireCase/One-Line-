// Production Edge Puzzle Foundation · Edge hit testing（纯函数）
...
// 3. 选择距离最近的有限 Edge segment；若最近与次近距离差 < tieEpsilon，返回 ambiguous
//    （点击不提交错误 Edge；拖动由手势层结合上一 Edge 与移动方向裁决）。
// 4. 全部候选中，横边/竖边分别给出最近距离，供 Hit Debug 展示。
//
// 参数来源：命中参数（corridor / tie epsilon）由 edgeGeometry 的 HIT_PARAMS 单一来源注入
// computeBoardLayout → layout.corridorHalfWidth / layout.tieEpsilon，本模块不重复定义。
...
/**
 * 从详细命中结果选择 hover / pointerdown 共用的目标 key（同一事实源）。
 * 命中且在 corridor 内且无歧义才返回 key；否则 null。
 */
export function selectHoverKey(detail, layout) {
  if (!detail || !detail.nearest) return null;
  if (detail.nearest.dist > layout.corridorHalfWidth) return null;
  if (detail.ambiguous) return null;
  return detail.nearest.key;
}
