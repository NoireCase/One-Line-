/**
 * Hidden / 隐迹连线 关卡数据（MVP demo，5×5，10 个手工关卡）
 *
 * 每关包含：
 *   id             — 稳定关卡标识
 *   title          — 玩家可见关卡名
...
const HIDDEN_LEVELS = [
  ...
  },
  {
    ...
    N: 7,
    path: [32,25,24,31,30,23,16,17,18,19,20,27,26,33,34,41,48,47,40,39,46,45,38,37,44,43,42,35,36,29,28,21,22,15,14,7,0,1,8,9,2,3,10,11,4,5,12,13,6],
    keyNumbers: [1,4,14,20,30,40,49],
    startIndex: 32,
    description: 'Hard 最终段入口，7-key 高压关。gapStd=2.6——key 间隔极度不均匀（3/10/6/10/10/9）。段 4→14（extra=8 close pair + long segment，10 步走 MD=2）是开局大陷阱——4 条路线各不相同。段 20→30（10 步 extra=6 long segment）后半段跨区。consecHigh=5。',
    difficultyLabel: 'Hard',
    archetypeTags: ['AREA_SPLIT'],
    shapeTag: 'MIXED'
  },
  {
    ...
    title: '六钥死锁',
    N: 7,
    path: [10,3,4,11,18,17,24,25,32,31,30,23,16,9,2,1,0,7,8,15,14,21,22,29,28,35,42,43,36,37,44,45,38,39,46,47,48,41,40,33,34,27,26,19,20,13,12,5,6],
    keyNumbers: [1,12,19,29,40,49],
    startIndex: 10,
    description: '6-key 区域死锁关。5 段，平均 9.6 步。段 1→12（11 步 extra=8 long segment）开局就是 11 步长段——没有任何短段缓冲。段 19→29（10 步 extra=6 long segment）是核心中轴穿越——有 3 条路线，每条锁定不同的后续区域。0 close pair。仅 6 个关键数字。',
    difficultyLabel: 'Hard',
    archetypeTags: ['CORNER_LOCK', 'AREA_SPLIT'],
    shapeTag: 'MIXED'
  },
  {
    ...
    title: '六钥暗阱',
    N: 7,
    path: [32,33,34,27,26,25,24,31,30,23,16,17,18,19,20,13,6,5,12,11,4,3,10,9,2,1,0,7,8,15,14,21,22,29,28,35,42,43,36,37,44,45,38,39,46,47,40,41,48],
    keyNumbers: [1,12,20,28,38,49],
    startIndex: 32,
    description: '6-key LATE_GAME_TRAP。5 段，平均 9.6 步。段 1→12（11 步 extra=8 long segment）开局长段——玩家倾向走最短路线，但最短路线会把后半段的必经区域填满。段 12→20（8 步 close pair extra=6）是检验点——此处暴露 1→12 是否走错。段 28→38（10 步）后期空间已被前期填满。consecHigh=5。',
    difficultyLabel: 'Hard',
    archetypeTags: ['LATE_GAME_TRAP'],
    shapeTag: 'MIXED'
  },
  {
    ...
    title: '六钥终哨',
    N: 7,
    path: [18,17,24,25,32,31,38,39,46,45,44,37,30,23,16,15,22,29,36,43,42,35,28,21,14,7,0,1,8,9,2,3,10,11,4,5,6,13,12,19,20,27,26,33,34,41,48,47,40],
    keyNumbers: [1,11,22,33,44,49],
    startIndex: 18,
    description: '6-key 最终前哨，BALANCED_DEDUCTION。5 段，平均 9.6 步。gapStd=2.3——key 间隔极度不均匀（10/11/11/11/5），最后一段 44→49 仅 5 步是极短收尾。三段连续 11 步段（11→22→33→44）构成全关核心——四区步数分配必须在段 1→11 就预判。',
    difficultyLabel: 'Hard',
    archetypeTags: ['BALANCED_DEDUCTION'],
    shapeTag: 'MIXED'
  },
  {
    ...
    title: '六钥终局',
    N: 7,
    path: [16,23,22,21,14,15,8,7,0,1,2,9,10,3,4,5,6,13,12,11,18,17,24,25,32,31,30,29,28,35,42,43,36,37,44,45,38,39,46,47,48,41,40,33,34,27,26,19,20],
    keyNumbers: [1,12,21,32,39,49],
    startIndex: 16,
    description: 'Hidden 终局大考，6-key 综合全部 Hard 技巧。5 段，平均 9.6 步。段 1→12（11 步 extra=10 close pair + long segment）——Hidden 60 关中最极端的单段：曼哈顿仅 1 格但需走 11 步！5 条路线可选，只有 1 条正确。综合 AREA_SPLIT（段 21→32 跨中轴）+ CORNER_LOCK（1 和 49 对角分布）+ LATE_GAME_TRAP（1→12 最短路线陷阱）+ BALANCED_DEDUCTION（五段步数全盘协调）。需要同时运用 Easy 关键数字分段、Medium 区域推理、Hard #31–#59 全部技巧。',
    difficultyLabel: 'Hard',
    archetypeTags: ['BALANCED_DEDUCTION'],
    shapeTag: 'MIXED'
  }
