      ...
      {ITEMS.map(item => (
        <button key={item.id} onClick={() => onUseItem(item.id)}
          ...
          <div className="item-token w-14 h-14 flex items-center justify-center relative bg-[#212430] border border-[#666170]/75">
            ...
            {items[item.id] > 0 ? (
              <span className="game-tool-button__count absolute -top-1.5 -right-1.5 bg-teal-700 text-teal-50 text-[10px] font-bold px-1.5 py-0.5 rounded-full">{items[item.id]}</span>
            ) : (
              <span className="game-tool-button__price absolute -bottom-1 bg-slate-900 text-slate-500 text-[10px] font-bold px-1 py-0 rounded-full border border-slate-700 flex items-center gap-0.5">
