...
export default function LosePanel({ isPortal2 = false, onRevive, onRestart, onBackToLevels, isDevCandidate, onDevAction }) {
  return (
    <div className="surface-panel p-7 max-w-sm w-full text-center animate-in zoom-in duration-200" data-testid="lose-panel">
      ...
      <h2 className="text-2xl font-bold text-[#e7d8c7] mb-2" data-testid="lose-title">
        {isDevCandidate ? '候选失败' : isPortal2 ? '路线卡住了' : '挑战失败'}
      </h2>
      <p className="text-[#857e72] text-sm">
        {isDevCandidate ? '路线中断。可以重玩或标记不合格。' : isPortal2 ? '重新规划一下路径。' : '路线中断了，再试一次。'}
      </p>
      ...
      {isDevCandidate ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-3 mt-2">
            ...
            <button onClick={onRestart} className="button-secondary flex-[1.5] py-3 flex justify-center items-center gap-1 text-sm" data-testid="lose-restart-button"><RotateCcw size={16} /> {isPortal2 ? '重新挑战' : '重新开始'}</button>
          </div>
        </div>
      )}
    </div>
