...
Playwright E2E tests live in `e2e/`. They cover home, mode selection, settings, mouse/keyboard input, save/restore, win/lose panels, Hidden, Star Line, Star Line Playtest Panel, and GM candidate review.
...
* No router, no state management library — gameplay state is orchestrated by `App.jsx` and the custom hooks in `src/hooks/`
...
```text
main.jsx → App.jsx (application orchestration)
...
| File                 | Responsibility                                                                                                             |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `gameModes.js`       | Play mode definitions (classic/diagonal/hidden/portalClassic/starLine), level structure, localStorage key helpers             |
...
| `star-line.spec.js`       | Star Line gameplay and level UI          |
| `star-line-teaching.spec.js` | Star Line teaching and completion copy |
| `star-line-playtest.spec.js` | Star Line Playtest Panel / GM tools    |

## Game Data Model

* **Board**: flat array of length `N*N`. Cell shape depends on the mode:
  * Classic / Diagonal / Hidden / Portal Classic: `{ val, isHidden, isRevealed, isExcluded, isHinted, portalId? }`
  * Star Line: `{ regionId, isStarred, isMarkedX }` plus solver metadata from `starLineLevels.js`
* **Path**: flat array of cell indices in visit order; in Classic/Portal Classic starts at the cell with `val: 1`. Star Line uses star positions instead of path progression.
* **Coordinates**: `row = Math.floor(index / N)`, `col = index % N`
* **Level configs**:

  * Classic / Diagonal levels total 60 each: base generated sections plus curated levels appended at the end of each mode/diff section
  * Hidden levels are hand-authored in `src/data/hiddenLevels.js` (60 total: Easy 5×5 10, Medium 7×7 20, Hard 7×7 30)
  * Portal Classic levels are hand-authored (30 levels, 5×5 and 7×7, with `path` + `hiddenVals` + `portals`)
  * Star Line levels are hand-authored (30 levels: Lv.1–20 单星, Lv.21–30 双星)
  * Portal Collect / Portal 2.0 was removed in v0.15.0 and is not a current playable mode

## Classic / Diagonal Level Generation

Generated Classic / Diagonal boards are created in `src/game/classic/createClassicLevel.js`.
...
4. Curated levels in `src/data/curatedLevels.js` are appended after generated sections and must not be inserted into the middle of existing sections

## Mode Structure

Five independent play modes, each with its own level list:

| Mode | Key | Movement | Levels | Board |
|------|-----|----------|--------|-------|
| 经典模式 | `classic` | Orthogonal (4-directional) | 60 | 5×5 / 7×7 / 9×9 |
| 八向连线 | `diagonal` | Diagonal (8-directional) | 60 | 5×5 / 7×7 / 9×9 |
| 极简线索 | `hidden` | Orthogonal (4-directional) | 60 | 5×5 / 7×7 |
| 经典传送门 / 空间传送 | `portalClassic` | Diagonal | 30 | 5×5 / 7×7 |
| 星线谜阵 | `starLine` | Star placement | 30 | 5×5 / 6×6 / 7×7 / 8×8 / 9×9 / 10×10 |

Classic and Diagonal share the same generated section structure and generation pipeline, then use curated levels to reach 60 levels per mode. Classic uses orthogonal movement throughout; Diagonal uses diagonal movement throughout. They are separate mode entries with independent progress tracking.

## Portal Modes

### Portal Classic (`portalClassic`)

Portal 1.0 rules: hidden numbers + sequential path + manual portal two-step. 30 hand-authored 5×5 and 7×7 levels with portal pairs.
...
Portal Collect was removed in v0.15.0 and should not be described as a current playable mode.

Historical Portal 2.0 rules were: free-order path + coin collection + auto-teleport portals + reach the exit.
...
* `cg_diagonal_highscores`
* `cg_diagonal_saved_game`
* `cg_hidden_progress`
* `cg_hidden_best_steps`
* `cg_hidden_saved_game`
* `cg_portal_*`
* `cg_star_line_progress`
* `cg_star_line_records`
* `cg_star_line_saved_game`
* `cg_discovery_star_line_basic_v1`
* `cg_discovery_star_line_double_star_v1`
* `cg_coins`
* `cg_items`
* `cg_global_score`
* `cg_sfx_vol`
...
* Cell states use Tailwind classes, not inline styles, switched by `getCellClass()` helper in `src/game/display/cellStyle.js`
