...
### P3A：玩法家族规范、接入合同与路线冻结

- 建立 `docs/game-family-design-system.md` 作为玩法家族设计、接入合同与 Go/No-Go 规则的权威来源。
- 对齐两个玩法家族、六个正式 modeId、330 个正式关卡及 legacy `starLine` 兼容边界。
- 明确当前平面 mode 配置与 P3B 计划的 family→mode 单一来源、runtime/session 接缝、声明式 schema / 输入 / 教学字段之间的边界。
- 冻结 P3B 约束：不建立万能玩法框架，不重写 `App.jsx`，不迁移无关存档，不同时实现新玩法。
...
- 建立两个家族的六种规则签名：One Line 四种签名（连续路径 / 遮蔽路径 / 折角路径 / 门环路径）、Star Line 两种签名（单核汇聚 / 双核共享）。
...
- One Line 与 Star Line 关卡内统一使用鼠标或触摸板操作
- 移除 One Line 的 WASD、方向键与键盘路径连接
- 移除设置中的键盘模式和相关操作文案
- 停止读取和写入旧 `cg_input_mode`
- 旧浏览器数据中的 `cg_input_mode` 保留但不再生效
- 游戏内棋盘、工具栏和结算操作不再支持键盘改变关卡状态
- 主页、设置页和关卡选择页仍保留浏览器原生按钮行为
...
- 同一次拖动重复经过同一格只处理一次
- 排除拖动不会覆盖已有星点
- 放置星点继续使用单击，不支持拖动连续放星
- 仅主指针和鼠标左键能够修改棋盘
- 右键、中键及非主指针不会修改棋盘

### 有限操作撤销

- Star Line 支持最近 20 次鼠标棋盘操作撤销
- 单击记为一步
- 一次拖动整体记为一步
- 工具切换不进入历史
- 重开、切关、退出和通关时清空历史
- 通关后撤销立即禁用
- 撤销历史不写入正式存档或 localStorage

### 项目标识

- 项目包名由 `my-game` 更新为 `one-line`
- 玩家可见页面品牌统一为 `Linebook`
- 页面标题和基础元信息同步更新

### 测试

- 增加右键、非主指针和失焦事务边界测试
- 增加真实 21 步操作与 20 步撤销上限测试
- 增加游戏内键盘不改变状态的反向测试
- 增加旧 `cg_input_mode=keyboard` 兼容测试
- 全量 E2E 扩展至 174 条
...
- HUD 显示剩余尝试次数
- 错误后即时更新剩余次数
- 最后一次错误保留棋盘反馈后再显示失败面板
- 失败文案明确说明尝试次数已用尽
- 重开、切关和卸载时正确清理延迟状态

### Star Line 结算节奏

- 单星关卡约 1000ms 后显示 WinPanel
- 双星关卡约 1300ms 后显示 WinPanel
- 双星逐星庆祝动画不再被结算面板提前遮挡
- 动画与面板等待使用统一时序配置

### 重新开始保护

- One Line 与 Star Line 有进度时需要二次确认
- 第一次点击不再立即清空进度
- 确认状态支持自动超时和 Escape 取消
- 支持 Enter / Space 键盘确认

### 提示与错误说明

- One Line 使用提示后显示简短下一步说明
- 提示按钮提前说明剩余次数或金币成本
- Portal 非法出口显示对应原因
- 通用路径错误提示从当前路径末端继续
- Hidden 与 Star Line 未新增答案型提示

### 测试

- 新增 Hidden 失败延迟测试
- 新增两种玩法重新开始确认测试
- 新增 One Line 提示与 Portal 错误说明测试
- 新增 Star Line 单星与双星结算时序测试
- 全量 E2E 扩展至 131 条

### 未修改范围

- 关卡数据
- 玩法规则和 Star Line quota
- solver / validator
- 存档格式和 localStorage key
- 解锁逻辑
- 单步撤销系统
- Star Line 答案提示
- 空间方向键导航
...
- 单星关卡支持 0/1、1/1 状态表达
- 双星关卡支持 0/2、1/2、2/2 状态表达
- 数量满足只表示达到规则数量，不代表位置一定属于最终答案
- 冲突状态优先替换局部数量反馈
- 多种冲突合并为紧凑的单行摘要
- 修复冲突信息与操作工具栏重叠
- 删除重复的通用错误提示
- 移除持续循环冲突脉冲，保留静态错误状态和首次轻微反馈
- 增强不同星域之间的几何边界
- 补充单星和双星教学说明
...
- 增加单星数量回退测试
- 增加双星 0/2、1/2、2/2 和超额测试
- 增加冲突优先级、多重冲突压缩和状态区布局测试
- 全量 E2E 扩展至 125 条

### 未修改范围

- Star Line quota 和玩法规则
- 关卡数据
- solver / validator
- 通关判定
- 存档格式及 localStorage key
- 解锁逻辑
- One Line 游戏组件
- 关卡选择页
- 移动端实验
...
- 重构 One Line 与 Star Line 关卡选择结构。
- 当前章节展开，已完成章节折叠并支持重玩。
- 锁定章节改为摘要展示。
- One Line 关卡卡片调整为更紧凑的横向布局。
- 章节 CTA 与推荐关卡、存档目标保持一致。
...
- ModeSwitcher 改为原生按钮组并使用 aria-pressed。
- 章节折叠使用稳定的 aria-expanded、aria-controls 和 hidden。
- 补齐五种模式的章节导航解析。
- 增加章节展开、重玩、星轨和键盘操作的 E2E 覆盖。
...
v0.19.0 桌面游戏体验收口：统一产品外壳、Game Shell 与 HUD 层级，加固通关与返回交互。本轮聚焦桌面视觉与交互一致性，不改玩法规则、关卡数据与存档结构。

### Changed

- **Linebook 产品外壳收口**：延续谜题书入口品牌层，统一玩法关卡页与 Linebook 入口视觉表现。
- **桌面 Game Shell / HUD 统一**：统一桌面游戏外壳、HUD 与棋盘舞台的层级与留白，收敛底部道具区。
- **Star Line 桌面棋盘优化**：优化星域棋盘桌面呈现、引导文案、工具栏顺序与整体居中。
- **Dev Candidate 独立左右栏**：保持 Dev Candidate 审核视图的独立左右栏布局与 Star Line 独立玩法外壳。

### Fixed

- **胜利遮罩防误触**：加固胜利面板遮罩，避免误触直接退出结算。
- **Star Line 退出确认**：有标记时离开增加确认，空局仍可直接返回。
- **Classic / Portal 桌面道具条**：修复桌面道具条隐藏问题，并补充 `e2e/game-shell.spec.js` 几何回归测试。

不涉及：玩法规则、solver / validator、关卡数据、存档 / 计分 / 奖励 / 解锁、依赖列表；不包含暂停中的移动端重构实验。
...
- **Star Line 基础验证**：easy 5×5 2 关 + medium 6×6 2 关 + hard 7×7 1 关，全部 solver 唯一解验证通过
...
- **交互工具**：放置星点 / 排除 X / 清除，X 为玩家笔记不参与通关判定。
...
- **Hidden 完成 60 关**：Easy 5×5 10 关 + Medium 7×7 20 关 + Hard 7×7 30 关
- **Medium 7×7 20 关**：覆盖 AREA_SPLIT / CORNER_LOCK / LATE_GAME_TRAP / BALANCED_DEDUCTION / MULTI_REGION_ROUTE 五类 archetype，key 密度 8–12
- **Hard 7×7 30 关**：6–7 key 高难，跨段后果链，最终段 #56–#60 为 6-key 强化版，#60 extra=10 作为 Hidden 终局大考
- **shape-tracing 风险检测**：`scripts/analyze-hidden-levels.mjs` 新增 SHAPE_SOLVABLE / ANCHOR_CHAIN / DOMINANT_SWEEP / SHAPE_TAG_OVERPOWER 检测
- **UI 适配**：7×7 棋盘自适应渲染，HUD 路径显示动态化（`/ 25` → `{N * N}`），HP 按 N 自适应（5×5=10, 7×7=15）
- **验证全通**：validate:hidden 4228/4228，validate:levels 361/361，build ✅，test 66/66
...
v0.14.0 Hidden / 极简线索 MVP：新增独立推理玩法雏形。

- **极简线索（Hidden）**：新增独立 GAME_MODE，5×5 demo，6 个手工关卡（3 引导 + 3 进阶推理）
- **分段关键数字推理**：棋盘仅显示 4–6 个关键数字，玩家按数字差值分段连接，每段移动次数由差值决定。进阶关卡引入 close pair 和长段推理
- **唯一解验证**：新增 `scripts/verify-hidden-unique.mjs`
- **独立存档**：`cg_hidden_progress` / `cg_hidden_saved_game`
- **独立视觉识别**：Hidden 使用独立 SVG 图案和 warm copper 色系
- **HUD / WinPanel 适配**：Hidden 专属结算，隐藏分数/星级/道具按钮
