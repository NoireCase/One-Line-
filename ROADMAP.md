...
- **P4C：数字环线桌面技术裁决（已完成 · GO WITH CHANGES）。** 裁决文档见 [`docs/p4c-digital-loop-technical-decision.md`](docs/p4c-digital-loop-technical-decision.md)：桌面技术方向成立、不需要返回输入 Spike、可进入正式生产化工程；**不允许把整个原型目录直接作为生产实现**，必须先提取通用 Edge Puzzle Foundation 再单独建设 Runtime / Validator；当前不得注册 familyId / modeId。GO **不表示**数字环线正式上线、对称分区已经验证、界环谜阵四玩法已经完成。
- **Package 1：Production Edge Puzzle Foundation（进行中）。** P4C 后的第一个生产化工程包：提取 `src/game/edgePuzzle/` 生产通用底座（edgeState / edgeCoordinates / edgeGeometry / hitTesting / gestureController / undoTransactions / edgeGraph），解决 P4C 指定必修问题（P2-1 按钮 Guard、P2-2 严格相邻、P2-3 活跃手势生命周期、原型动态加载、测试接入 npm script 与 Quality Gate）；原型改为消费生产底座；本包不建设数字环线正式玩法、不注册正式 mode。
